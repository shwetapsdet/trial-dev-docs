...
import { data } from '../../wdio.base.conf.ts'
import config from '../../wdio.base.conf.ts';
const fs = require('fs');
const dns = require('node:dns');

import { InstancePicker } from "../../utils/InstancePicker.ts";
import { Paths } from "../../utils/path.ts";
import { context } from "../../utils/context.ts";
import { WAIT_FOR_TIMEOUT } from "../../core/timeouts.ts";
...
config.beforeSession = async function (config, capabilities, specs, cid) {
  ...
  const { instanceNumber, failedScenarios } = JSON.parse(fs.readFileSync(`${Paths.worker_context_directory}/${cid}`));
  ...
  let preparingStr = `PREPARING /tests/sg/${specs[0].split('/src/').pop()} (${username} - ${instanceNumber})`;
...
config.afterSession = function (config, capabilities, specs) {
  ...
  try {
    ...
    fs.rmSync(`${Paths.worker_context_directory}/${cid}`);
